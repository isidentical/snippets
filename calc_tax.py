...
def calculate_base_tax(income):
    if income > _INCOME_THRESHOLD:
        ...
        return calculate_base_tax(_INCOME_THRESHOLD) + (
            income - _INCOME_THRESHOLD
        ) * 32 / 100
    else:
...
def main():
    ...
    parser.add_argument("annual_income", type=int, help="Annual income in zlotys")
    ...
    print('Net monthly income:', net_annual_income / 12, 'zl', f'(taxed @ {total_tax/12} zl per month)')
